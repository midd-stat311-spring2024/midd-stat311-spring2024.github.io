...
$$X_{1},\ldots, X_{10} |\theta \sim \text{Bern}(\theta), \qquad \theta \in [0,1]$$
$$H_{0}: \theta = \frac{1}{2} \qquad \text{vs.} \qquad H_{1}: \theta \neq \frac{1}{2}$$
We have the following testing procedure $\delta_{1}$:
$r(\mathbf{X}) = |\sum_{i=1}^{10} X_{i} - 5|, \text{ with rejection region } R = \{r(\mathbf{X}) = 5 \}$
...
Consider the second testing procedure $\delta_{2}$:
$r(\mathbf{X}) = |\sum_{i=1}^{10} X_{i} - 5| \text{ with rejection region } R = \{r(\mathbf{X}) \geq 4 \}$.
